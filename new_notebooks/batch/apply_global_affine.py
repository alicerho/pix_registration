...
# # GLOBAL_AFFINE = Path("green_to_FITC_global_affine.txt")
# # GLOBAL_AFFINE = Path("red_to_TRITC_global_affine.txt")
# # GLOBAL_AFFINE = Path("yellow_to_YFP_global_affine.txt")
...
#     # pattern = "unmixed_EYrainbow_slide-*_field-*_spectral-green.nd2"
#     # pattern = "unmixed_EYrainbow_slide-*_field-*_spectral-red.nd2"
#     # pattern = "unmixed_EYrainbow_slide-*_field-*_spectral-yellow.nd2"
...
#         # CHECK WHAT SHAPE THE UNMIXED FILE IS
#         print(f"  DEBUG: Unmixed array shape = {arr.shape}")
#         print(f"  DEBUG: Unmixed array dtype = {arr.dtype}")
...
#         print(f"  DEBUG: px shape after extraction = {px.shape}")
#         print(f"  DEBUG: vo shape after extraction = {vo.shape}")

#         cam_path = RAW / f"EYrainbow_slide-{slide}_field-{field}_camera-DAPI.nd2"
#         # cam_path = RAW / f"EYrainbow_slide-{slide}_field-{field}_camera-FITC.nd2"
#         # cam_path = RAW / f"EYrainbow_slide-{slide}_field-{field}_camera-TRITC.nd2"
#         # cam_path = RAW / f"EYrainbow_slide-{slide}_field-{field}_camera-YFP.nd2"
...
#         # save_tif(OUT / f"{prefix}_spectral-mt.tif", px_w)
#         # save_tif(OUT / f"{prefix}_spectral-ld.tif", vo_w) 
#         # save_tif(OUT / f"{prefix}_spectral-mt.tif", px_w)
#         # save_tif(OUT / f"{prefix}_spectral-ld.tif", vo_w) 
...
CONFIG = {
    "blue": {
        "dir": UNMIXED,
        "pattern": "unmixed_EYrainbow_slide-*_field-*_spectral-blue.nd2",
        "affine": "blue_to_DAPI_global_affine.txt",
        "camera": "DAPI",
        "channels": ["px", "vo"],
    },
    "red": {
        "dir": UNMIXED / "red",
        "pattern": "unmixed_EYrainbow_slide-*_field-*_spectral-red.nd2",
        "affine": "red_to_TRITC_global_affine.txt",
        "camera": "TRITC",
        "channels": ["mt", "ld"],
    },
    "yellow": {
        ...
        "affine": "yellow_to_YFP_global_affine.txt",
        "camera": "YFP",
        "channels": ["go"],
    },
    "green": {
        ...
        "affine": "green_to_FITC_global_affine.txt",
        "camera": "FITC",
        "channels": ["er"],
    },
}
...
def main():
    ...
    for f in files:
        ...
        # LOAD CAMERA
        # ------------------------
        cam_path = RAW / f"EYrainbow_slide-{slide}_field-{field}_camera-{cfg['camera']}.nd2"
        cam = load_nd2(cam_path)
        ...
        for name, img in outputs:
            ...
            # save tif
            out_path = OUT / f"{prefix}_spectral-{name}.tif"
            save_tif(out_path, img_w)
            print(f"  Saved {name}: {img_w.shape}")

            # overlay before/after
            fig, axes = plt.subplots(1, 2, figsize=(12, 6))

            axes[0].imshow(overlay_rgb(cam_cropped, img))
            axes[0].set_title(f"BEFORE\nRed=Camera, Green={name}")
            axes[0].axis("off")

            axes[1].imshow(overlay_rgb(cam_cropped, img_w))
            axes[1].set_title(f"AFTER\nRed=Camera, Green={name}")
            axes[1].axis("off")

            plt.tight_layout()
            plt.savefig(OUT / f"{prefix}_overlay_before_after_{name}.png",
            ...
            fig, ax = plt.subplots(1, 3, figsize=(15, 5))
            ax[0].imshow(norm(cam_cropped), cmap="gray")
            ax[0].set_title(f"Camera {cfg['camera']}")
            ax[1].imshow(norm(img), cmap="gray")
            ax[1].set_title(f"Raw {name}")
            ax[2].imshow(norm(img_w), cmap="gray")
            ax[2].set_title(f"Warped {name}")
            for a in ax:
                a.axis("off")
            plt.tight_layout()
            ...
            plt.axis("off")
            plt.title(f"Checkerboard: {cfg['camera']} vs {name}")
            plt.savefig(OUT / f"{prefix}_checkerboard_{name}.png", dpi=200)
            plt.close()

        print("  Saved TIFFs and QC images")
