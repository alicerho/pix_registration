...
#     # Load saved warped images
#     fixed = ants.image_read("outputs_FOV6_blue_to_DAPI/fixed_camera.nii.gz").numpy()
#     aff   = ants.image_read("outputs_FOV6_blue_to_DAPI/warped_affine.nii.gz").numpy()
#     syn   = ants.image_read("outputs_FOV6_blue_to_DAPI/warped_affine_syn.nii.gz").numpy()
...
#     print("Mean bead displacement after Affine:", np.mean(dist_aff))
#     print("Mean bead displacement after Syn:", np.mean(dist_syn))

#     # ---------------------------------------------------
#     # Composite visualization (5 panels)
#     # ---------------------------------------------------
...
#     axes[0].set_title("Fixed Camera")
#     axes[0].axis("off")

#     # ------------ (2) AFFINE WARP ------------
#     axes[1].imshow(aff, cmap="gray")
#     axes[1].set_title("Affine Warped Spectral")
#     axes[1].axis("off")

#     # ------------ (3) SYN WARP ------------
#     axes[2].imshow(syn, cmap="gray")
#     axes[2].set_title("SyN Warped Spectral")
#     axes[2].axis("off")

#     # ------------ (4) Bead overlay: fixed vs affine ------------
#     axes[3].imshow(fixed, cmap="gray")
#     axes[3].scatter(beads_fixed[:,1], beads_fixed[:,0], s=10, c='lime', label="fixed")
#     axes[3].scatter(beads_aff[:,1], beads_aff[:,0], s=10, c='red', alpha=0.5, label="affine")
#     axes[3].set_title(f"Beads: Fixed vs Affine\nMean dist = {np.mean(dist_aff):.2f}px")
#     axes[3].axis("off")
#     axes[3].legend(loc="lower right", fontsize=8)

#     # ------------ (5) Bead overlay: fixed vs syn ------------
#     axes[4].imshow(fixed, cmap="gray")
#     axes[4].scatter(beads_fixed[:,1], beads_fixed[:,0], s=10, c='lime', label="fixed")
#     axes[4].scatter(beads_syn[:,1], beads_syn[:,0], s=10, c='magenta', alpha=0.5, label="syn")
#     axes[4].set_title(f"Beads: Fixed vs SyN\nMean dist = {np.mean(dist_syn):.2f}px")
#     axes[4].axis("off")
#     axes[4].legend(loc="lower right", fontsize=8)

#     # Leave the 6th slot empty, or add future metrics
#     axes[5].axis("off")

#     fig.suptitle("Registration Evaluation Summary", fontsize=16)
#     plt.tight_layout()

#     # SAVE TO PNG
#     plt.savefig("outputs_FOV6_blue_to_DAPI/evaluation_summary.png", dpi=300)
#     plt.show()

#     print("\nSaved composite summary to evaluation_summary.png!")
